...
export class DatabaseManager {
  ...
  private async createTables(): Promise<void> {
    ...
    try {
      await this.db.exec(
        ALTER TABLE matches ADD COLUMN riot_id TEXT;
      );
    } catch (error) {
      // Column might already exist
    }
    ...
    try {
      await this.db.exec(
        ALTER TABLE matches ADD COLUMN pick_ban_data TEXT;
      );
    } catch (error) {
      // Column might already exist
    }
    ...
    try {
      await this.db.exec(
        ALTER TABLE matches ADD COLUMN detected_by_lcu INTEGER DEFAULT 0;    // Add new columns for match linking if they don't exist
    try {
      await this.db.exec(`
        ALTER TABLE matches ADD COLUMN riot_game_id TEXT;
      `);
    ...
    try {
      await this.db.exec(
        ALTER TABLE matches ADD COLUMN detected_by_lcu INTEGER DEFAULT 0;
      );
    } catch (error) {
      // Column might already exist
    }
  ...
  // Método para deletar uma partida específica
  async deleteMatch(matchId: number): Promise<void> {
    if (!this.db) throw new Error('Banco de dados não inicializado');
    
    try {
      await this.db.run('DELETE FROM matches WHERE id = ?', [matchId]);
      console.log(`🗑️ Partida ${matchId} deletada do banco de dados`);
    } catch (error) {
  ...
  async updateMatchWithResult(matchId: number, updateData: any): Promise<void> {
    if (!this.db) throw new Error('Banco de dados não inicializado');

    console.log('📝 [DatabaseManager] Atualizando partida com dados completos:', matchId, updateData);

    // Construir query dinâmica baseada nos campos fornecidos
    const fields = [];
    const values = [];

    if (updateData.winner_team !== undefined) {
      fields.push('winner_team = ?');
      values.push(updateData.winner_team);
    }

    if (updateData.completed !== undefined) {
      fields.push('status = ?');
      values.push(updateData.completed ? 'completed' : 'pending');
    }
    ...
    if (updateData.detected_by_lcu !== undefined) {
      fields.push('detected_by_lcu = ?');
      values.push(updateData.detected_by_lcu ? 1 : 0);
    }
    ...
    if (updateData.riot_id !== undefined) {
      fields.push('riot_id = ?');
      values.push(updateData.riot_id);
    }

    if (updateData.pick_ban_data !== undefined) {
      fields.push('pick_ban_data = ?');
      values.push(updateData.pick_ban_data);
    }

    if (fields.length === 0) {
      console.warn('⚠️ [DatabaseManager] Nenhum campo para atualizar');
      return;
    }

    values.push(matchId); // WHERE condition
    
    const query = `UPDATE matches SET ${fields.join(', ')} WHERE id = ?`;
    
    console.log('🔧 [DatabaseManager] Query de atualização:', query);
    console.log('📊 [DatabaseManager] Valores:', values);

    await this.db.run(query, values);

    console.log('✅ [DatabaseManager] Partida atualizada com sucesso');
  }
}
